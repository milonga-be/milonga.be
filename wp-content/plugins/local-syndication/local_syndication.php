<?php
/*
Plugin Name: Local Syndication
Plugin URI: http://www.willmaster.com/software/WPplugins/
Description: Insert content of any page/file/script at any URL on the Internet into any WordPress post or page.
Version: 1.4
...
/*
	Copyright 2011, 2013 Will Bontrager Software LLC (email: will@willmaster.com)
...
if( ! function_exists('will_bontrager_local_syndication') )
{
	function will_bontrager_local_syndication($atts,$content=null,$code="")
	{
		$url = $atts['url'];
		...
		if( $iframe )
		{
			...
			if( $js )
			{
				$pre_result .= will_bontrager_local_syndication_js_line($s);
				$post_result = will_bontrager_local_syndication_js_line($post_result).'</script>';
			}
			else { $pre_result .= $s; }
		...
		if( $plain )
		{
			...
			{
				$pre_result .= will_bontrager_local_syndication_js_line($s);
				$post_result = will_bontrager_local_syndication_js_line($post_result);
			}
			else { $pre_result .= $s; }
		} # if( $plain )
		elseif( ! $iframe )
		{
			...
			if( ! empty($baseurl) )
			{
				...
				will_bontrager_local_syndication_normalize_urls($basepieces, $http, $domain, $matches, $response_content);
		...
		{ 
			$post_result .= '</script>';
			$response_content = will_bontrager_local_syndication_js_block($response_content);
		}
		return "$pre_result$response_content$post_result";
	} # function will_bontrager_local_syndication()

	function will_bontrager_local_syndication_normalize_urls($basepieces, $http, $domain, $matches, &$response_content)
	{
		$done = array();
		...
		for( $indice=0; $indice<$count; $indice++ )
		{
			...
			$done[$relurl] = 1;
			$absurl = will_bontrager_local_syndication_make_absolute($basepieces, $http, $domain, $relurl);
	...
	} # function will_bontrager_local_syndication_normalize_urls()

	function will_bontrager_local_syndication_make_absolute($basepieces, $http, $domain, $url)
	{
 		$bpieces = $basepieces;
	...
	function will_bontrager_local_syndication_js_line($s)
	{
		$s = str_replace("\\","\\"."\\",$s);
		$s = str_replace("'","\\"."'",$s);
		$s = str_replace('</',"<'+'/",$s);
	...
	function will_bontrager_local_syndication_js_block(&$s)
	{
		$s2 = '';
		$s = str_replace("\r",'',$s);
	...
	add_shortcode('syndicate_local','will_bontrager_local_syndication');
