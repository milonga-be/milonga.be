	...
	if($pagination !== 'none') {
		...
		if ($num_pages > 1) {
			...
			if ($pagination === 'numbers') {
				...
				$navigation .= '<div class="page-links">'
				...
				if ($low_num > 1) {
					$navigation .= '<a href="' . add_query_arg('page', ($low_num - 1), $permalink) . '"><span>...</span></a> ';
				}

				for ($i = $low_num; $i <= $high_num; $i++) {
					if ($i == $page_num) $navigation .= '<span class="page-links-here">' . $i . '</span> ';
					else {
						$navigation .= '<a href="' . add_query_arg('page', $i, $permalink) . '"><span>' . $i . '</span></a> ';
					}
				}

				if ($high_num < $num_pages) {
					$navigation .= '<a href="' . add_query_arg('page', ($high_num + 1), $permalink) . '"><span>...</span></a> ';
				}
